...
#                                                                                               #
# @file   : FDD_Config.py                                                                       #
# @brief  : This Script will call corresponding functions and Operation related to FDD Conformance Test Cases. #
...
def Base_DL(bands, eAxID, phase_comp, band_folder, DL_Center_Freq, Power_Value, VXT_Add, Ext_Gain):
    ...
    if not os.path.exists(sub_folder):
        os.mkdir(sub_folder)
    bit_width = 16
    Generate_SCP.Base_DL_FDD(bands, phase_comp, sub_folder, DL_Center_Freq, Power_Value, VXT_Add)
    Generate_PCAP_ORB.DL(bands, eAxID, sub_folder, bit_width)
    Rest_API_Script.load_play_record_pcap(sub_folder)
    VXT_Script.Constellation_check_DL_FDD(bands, sub_folder, DL_Center_Freq, Power_Value, VXT_Add, Ext_Gain)
...
def Extended_DL(bands, eAxID, phase_comp, band_folder, DL_Center_Freq, Power_Value, VXT_Add, Ext_Gain):
    ...
    if not os.path.exists(sub_folder):
        os.mkdir(sub_folder)
    bit_width = 16
    Generate_SCP.Extended_DL_FDD(bands, phase_comp, sub_folder, DL_Center_Freq, Power_Value, VXT_Add)
    Generate_PCAP_ORB.DL(bands, eAxID, sub_folder, bit_width)
    Rest_API_Script.load_play_record_pcap(sub_folder)
    VXT_Script.Constellation_check_DL_FDD_repeat(bands, sub_folder, DL_Center_Freq, Power_Value, VXT_Add, Ext_Gain)
    Rest_API_Script.Stop_Player()
...
def Base_UL(bands, eAxID, phase_comp, band_folder, UL_Center_Freq, Power_Value, VXT_Add):
    ...
    if not os.path.exists(sub_folder):
        os.mkdir(sub_folder)
    bit_width = 16
    Generate_SCP.Base_UL_FDD(bands, phase_comp, sub_folder, UL_Center_Freq, Power_Value, VXT_Add)
...
def No_Compression_DL(bands, eAxID, phase_comp, band_folder, DL_Center_Freq, Power_Value, VXT_Add, Ext_Gain):
    ...
    if not os.path.exists(sub_folder):
        os.mkdir(sub_folder)
    bit_width = 16
    Generate_SCP.Compression_FDD_DL(bands, phase_comp, sub_folder, DL_Center_Freq, Power_Value, VXT_Add)
    Generate_PCAP_ORB.DL(bands, eAxID, sub_folder, bit_width)
    Rest_API_Script.load_play_record_pcap(sub_folder)
    VXT_Script.Constellation_check_DL_FDD_Comp_1(bands, sub_folder, DL_Center_Freq, Power_Value, VXT_Add, Ext_Gain)
    Rest_API_Script.Stop_Player()
...
def No_Compression_UL(bands, eAxID, phase_comp, band_folder, UL_Center_Freq, Power_Value, VXT_Add):
    ...
    if not os.path.exists(sub_folder):
        os.mkdir(sub_folder)
    bit_width = 16
    Generate_SCP.Compression_FDD_UL(bands, phase_comp, sub_folder, UL_Center_Freq, Power_Value, VXT_Add)
    Generate_PCAP_ORB.UL(bands, eAxID, sub_folder, bit_width)
    Rest_API_Script.load_play_record_pcap(sub_folder)
    Rest_API_Script.Stop_Player()
    Generate_PCAP_ORB.Generate_ORB(sub_folder, eAxID)
...
def FDD(band_count, bands, eAxID, phase_comp, Base_Folder_Path, DL_Center_Freq, UL_Center_Freq, Power_Value, VXT_Add, Ext_Gain):
    ...
    for i in range(0,band_count):
        ...
        band_name = "FDD_"+str(bands[i])+"_"+str(Power_Value)+"_"+str(eAxID)
